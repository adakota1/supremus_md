            ...
            switch (cmd) {
                ...
                case 'tagall': {
                    try {
                        ...
                        let participants = metadata.participants;
                        // Extraire le message à envoyer à tout le monde (après la commande)
                        let textToTag = message.message.conversation.substring(7).trim();
                
                        // Construire le message avec les mentions de tous les participants
                        let taggedMessage = '';
                        let mentionJid = [];
                        for (let participant of participants) {
                            // Ajouter l'identifiant à la 
                            mentionJid.push(participant.id);
                            // Ajouter le texte de mention
                            taggedMessage += `@${participant.id.split('@')[0]} `;
                        }
                
                        // Envoyer le message avec les mentions
                        await socket.sendMessage(message.key.remoteJid, { text: `${textToTag}\n${taggedMessage}`, mentions: mentionJid }, { quoted: message });
                    } catch (error) {
                        console.error("Erreur lors de l'exécution de la commande tagall :", error);
                        reply("Erreur lors de l'exécution de la commande tagall.");
                    }
                    break;
                }
                case 'all': {
